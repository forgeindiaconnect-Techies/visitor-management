...
const Header = ({ toggleSidebar, isSidebarOpen }) => {
  ...
  useEffect(() => {
    const loadCompanyBranding = async () => {
      const token = localStorage.getItem('token') || localStorage.getItem('adminToken');
      if (!token || user?.role === 'SaaS Super Admin') return;
      ...
      try {
        const response = await fetch(
          `${rawBaseUrl}/api/companies/me/branding`,
          {
            headers: {
              Authorization: `Bearer ${token}`
            }
        ...
        if (response.ok) {
          ...
          if (result.data?.branding?.primaryColor) {
            document.documentElement.style.setProperty('--color-brand-indigo', result.data.branding.primaryColor);
          }
        }
      } catch (err) {
        console.error('Unable to load company branding:', err);
      }
    };
